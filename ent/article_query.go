...
func (aq *ArticleQuery) loadFeed(ctx context.Context, query *FeedQuery, nodes []*Article, init func(*Article), assign func(*Article, *Feed)) error {
	...
	for i := range nodes {
		if nodes[i].feed_articles == nil {
			continue
		}
		fk := *nodes[i].feed_articles
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "feed_articles" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *ArticleQuery) loadSummary(ctx context.Context, query *SummaryQuery, nodes []*Article, init func(*Article), assign func(*Article, *Summary)) error {
	...
	query.Where(predicate.Summary(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(article.SummaryColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.article_summary
		if fk == nil {
			return fmt.Errorf(`foreign-key "article_summary" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "article_summary" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
